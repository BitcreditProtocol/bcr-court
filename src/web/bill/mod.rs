...
pub async fn receive_shared_bill(
    ...
    let decrypted_bill = match decrypt_bill(content, &secret_key) {
        Ok(bill) => bill,
        Err(e) => {
...
fn decrypt_bill(
    shared_bill: &BillToShareWithExternalParty,
    secret_key: &SecretKey,
) -> Result<Vec<BillBlockPlaintextWrapper>, anyhow::Error> {
    let decrypted =
        util::crypto::decrypt_ecies(&util::base58_decode(&shared_bill.data)?, secret_key)?;
